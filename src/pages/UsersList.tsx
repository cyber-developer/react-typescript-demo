...
const Dashboard = () => {
  ...
  const handleSubmit = () => {
    if (name) {
      let index = users.findIndex(n => n === name);
      if (index == -1) {
        const newList = [...users];
        newList.push(name);
        setUsers(newList);
  ...
  const footerLayout = () => {
    return <Button type="ghost" onClick={() => selectRandom()} >Select Random</Button>
  }
    ...
    <div>
      <h1>LIST OF NAMES</h1>
        <List
          ...
          renderItem={(item, key) => (
            <List.Item
              actions={[<Button onClick={() => handleDelete(key)} type="danger" icon={<DeleteFilled />} />]}
            >
              <List.Item.Meta
                description={item}
              />
